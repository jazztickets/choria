...
void _Scripting::InjectStats(const _Stats *Stats) {
	...
	for(const auto &Iterator : Stats->DamageTypes) {
		...
		lua_pushstring(LuaState, Iterator.second.second.c_str());
		lua_pushstring(LuaState, Iterator.second.first.c_str());
...
void _Scripting::InjectItems(const _Stats *Stats) {
	...
	for(const auto &Item : Stats->Items) {
		...
		lua_pushstring(LuaState, Item.first.c_str());
		...
		lua_pushstring(LuaState, Item.second.Name.c_str());
		lua_setfield(LuaState, -2, "Name");

		lua_pushinteger(LuaState, Item.second.Cost);
