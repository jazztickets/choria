...
void _OldClientState::HandlePacket(ENetEvent *Event) {
...
// Key events
void _OldClientState::KeyEvent(const _KeyEvent &KeyEvent) {
...
// Mouse events
void _OldClientState::MouseEvent(const _MouseEvent &MouseEvent) {
...
// Window events
void _OldClientState::WindowEvent(uint8_t Event) {
	if(Camera && Event == SDL_WINDOWEVENT_SIZE_CHANGED)
		Camera->CalculateFrustum(Graphics.AspectRatio);
}

// Handle an input action
bool _OldClientState::HandleAction(int InputType, int Action, int Value) {
	if(Value == 0)
		return true;
...
void _OldClientState::Update(double FrameTime) {
...
void _OldClientState::Render(double BlendFactor) {
...
void _OldClientState::SendBusy(bool Value) {
	_Buffer Packet;
...
void _OldClientState::HandleYourCharacterInfo(_Buffer *Packet) {
...
void _OldClientState::HandleChangeMaps(_Buffer *Packet) {
	if(!Player)
		return;
...
// Creates an object
void _OldClientState::HandleCreateObject(_Buffer *Packet) {
...
// Deletes an object
void _OldClientState::HandleDeleteObject(_Buffer *Packet) {
	int NetworkID = Packet->Read<char>();
...
void _OldClientState::HandleObjectUpdates(_Buffer *Packet) {
...
void _OldClientState::HandleStartBattle(_Buffer *Packet) {
...
void _OldClientState::HandleBattleTurnResults(_Buffer *Packet) {
...
void _OldClientState::HandleBattleEnd(_Buffer *Packet) {
...
void _OldClientState::HandleBattleCommand(_Buffer *Packet) {
...
// Handles HUD updates
void _OldClientState::HandleHUD(_Buffer *Packet) {
	Player->Experience = Packet->Read<int32_t>();
	Player->Gold = Packet->Read<int32_t>();
...
void _OldClientState::HandlePlayerPosition(_Buffer *Packet) {
	Player->Position.x = Packet->Read<char>();
	Player->Position.y = Packet->Read<char>();
}

// Handles the start of an event
void _OldClientState::HandleEventStart(_Buffer *Packet) {
	int Type = Packet->Read<char>();
...
void _OldClientState::HandleInventoryUse(_Buffer *Packet) {
	int Slot = Packet->Read<char>();
	Player->UpdateInventory(Slot, -1);
}

// Handles a chat message
void _OldClientState::HandleChatMessage(_Buffer *Packet) {
...
void _OldClientState::HandleTradeRequest(_Buffer *Packet) {
...
void _OldClientState::HandleTradeCancel(_Buffer *Packet) {
...
void _OldClientState::HandleTradeItem(_Buffer *Packet) {
...
void _OldClientState::HandleTradeGold(_Buffer *Packet) {
...
void _OldClientState::HandleTradeAccept(_Buffer *Packet) {
...
void _OldClientState::HandleTradeExchange(_Buffer *Packet) {
...
void _OldClientState::SendMoveCommand(int Direction) {
...
void _OldClientState::SendAttackPlayer() {
