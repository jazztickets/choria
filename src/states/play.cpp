...
void _PlayState::HandleActionResults(ae::_Buffer &Data) {
	...
	if(ItemUsed) {
		if(SourceObject && !SkillUnlocked && ItemUsed->Cooldown > 0.0) {
			SourceObject->Character->Cooldowns[ItemUsed->ID].Duration = ItemUsed->Cooldown * SourceObject->Character->Attributes["Cooldown"].Mult();
			SourceObject->Character->Cooldowns[ItemUsed->ID].MaxDuration = ItemUsed->Cooldown * SourceObject->Character->Attributes["Cooldown"].Mult();
		}
