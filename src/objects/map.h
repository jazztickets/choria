...
const uint16_t MAP_ZONE_COLORS = 6;
...
class _Map : public ae::_BaseObject, public micropather::Graph {
	...
	public:
		...
		void RenderProps(std::list<_Object *>::iterator &Iterator, const glm::vec2 &CollisionPoint, const ae::_Program *Program, glm::vec4 &Bounds, bool Floor);
		void Render2D(ae::_Camera *Camera);
		void AddLights(const glm::vec2 &CollisionPoint, const std::list<_Object *> *ObjectList, const ae::_Program *Program, glm::vec4 AABB);
	...
	private:
		...
		// Tiles
		void GetTransition(_Tile &Tile, const glm::ivec2 &CheckPosition, std::map<uint32_t, std::pair<int, uint32_t> > &Pairs, uint32_t Bit);
