...
void _Object::Render(glm::vec4 &ViewBounds, const _Object *ClientPlayer) {
	...
	if(!Character->Invisible || SameParty) {
		...
		ae::Assets.Fonts["hud_medium"]->DrawTextFormatted(Prefix + NameText, NamePosition + glm::vec2(0, OffsetY), ae::CENTER_BASELINE, 1.0f, 1.0f / (UI_TILE_SIZE.x * UIScale));
	}
}
...
void _Object::ApplyDeathPenalty(bool InBattle, double Penalty, int64_t BountyLoss) {
	...
	if(Server) {
		...
		if(Peer) {
			...
			Buffer << "and lost " << GoldPenalty << " gold";
