...
void _Map::GetPotentialBattlePlayers(const _Object *Player, float DistanceSquared, size_t Max, std::list<_Object *> &Players) {
	...
	for(const auto &Object : Objects) {
		...
		if(Object->Character->Attributes["Rebirths"].Integer != Player->Character->Attributes["Rebirths"].Integer || std::abs(Object->Character->Level - Player->Character->Level) > BATTLE_LEVEL_RANGE)
			continue;
...
void _Map::GetPVPPlayers(const _Object *Attacker, std::list<_Object *> &Players, bool UsePVPZone) {
	...
	for(const auto &Object : Objects) {
		...
		if(!UsePVPZone && !Object->Character->Attributes["Bounty"].Integer)
			continue;
		...
		// Check for rank
		if(Object->Character->Attributes["Rebirths"].Integer != Attacker->Character->Attributes["Rebirths"].Integer)
			continue;
...
_Object *_Map::FindTradePlayer(const _Object *Player, float MaxDistanceSquared) {
	...
	for(const auto &Object : Objects) {
		...
		if(Object->Character->Attributes["Rebirths"].Integer != Player->Character->Attributes["Rebirths"].Integer)
			continue;
