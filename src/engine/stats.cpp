...
// Loads skill types
void StatsClass::LoadSkillTypes() {

	// Clear types
	SkillTypes.clear();
	
	// Run query
	Database->RunDataQuery("SELECT * FROM SkillTypes");

	// Get events
	SkillTypeStruct SkillType;
	while(Database->FetchRow()) {
		SkillType.Name = Database->GetString(1);
...
void StatsClass::LoadSkills() {
	...
	while(Database->FetchRow()) {
		...
		Skill.WeaponDamagePercent = Database->GetFloat(15);
		//Skill.ManaCost = Database->GetFloat(5);
...
void StatsClass::GetMonsterStats(int MonsterID, MonsterClass *Monster) {
	...
	if(Database->FetchRow()) {
		...
		Monster->Portrait = irrDriver->getTexture((stringc("textures/monsters/") + stringc(Database->GetString(3))).c_str());
		...
		Monster->ActionBar[0] = Stats.GetSkill(0);
	}
...
void StatsClass::GetPortraitList(list<PortraitStruct> &List) {
...
void StatsClass::GenerateMonsterListFromZone(int ZoneID, array<int> &Monsters) {
	if(ZoneID == 0)
		return;
	...
	if(OddsSum > 0) {
		...
		for(int i = 0; i < MonsterCount; i++) {
			RandomNumber = Random.GenerateRange(1, OddsSum);
...
void StatsClass::GenerateMonsterDrops(int MonsterID, int Count, array<int> &Drops) {
	if(MonsterID == 0)
		return;
...
const LevelStruct *StatsClass::FindLevel(int Experience) const {
...
// Gets a skill from an ID
const SkillClass *StatsClass::GetSkill(int SkillID) {
	std::map<int, SkillClass>::iterator SkillIterator;
	SkillIterator = Skills.find(SkillID);
