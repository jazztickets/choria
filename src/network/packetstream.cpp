...
PacketClass::PacketClass(char PacketType, ENetPacketFlag Flag, enet_uint8 Channel)
:	CurrentByte(0),
	CurrentBit(0),
	Channel(Channel) {

	// Create the enet packet
	Packet = enet_packet_create(NULL, 32, Flag);
...
PacketClass::PacketClass(ENetPacket *Packet)
:	CurrentByte(0),
	CurrentBit(0),
	Packet(Packet) {
...
void PacketClass::AlignAndExpand(int NewWriteSize) {
	AlignBitIndex();

	// Resize the packet if needed
	size_t NewPacketSize = CurrentByte + NewWriteSize;
	if(NewPacketSize > Packet->dataLength)
		enet_packet_resize(Packet, NewPacketSize << 1);
}

// Writes a single bit to the stream
void PacketClass::WriteBit(bool Data) {
	...
	// Write the bit
	Packet->data[CurrentByte] |= Data << CurrentBit;
...
void PacketClass::WriteChar(char Data) {
	AlignAndExpand(1);

	Packet->data[CurrentByte] = Data;
	CurrentByte++;
}

// Write a single int to the stream
void PacketClass::WriteInt(int Data) {
	AlignAndExpand(sizeof(int));

	*((int *)&Packet->data[CurrentByte]) = Data;
	CurrentByte += sizeof(int);
}

// Write a single float to the stream
void PacketClass::WriteFloat(float Data) {
	AlignAndExpand(sizeof(float));

	*((float *)&Packet->data[CurrentByte]) = Data;
	CurrentByte += sizeof(float);
}

// Write a string to the stream
void PacketClass::WriteString(const char *Data) {
	int StringLength = strlen(Data);
	AlignAndExpand(StringLength + 1);

	// Copy string to stream
	strcpy((char *)&Packet->data[CurrentByte], Data);
